...
class Discriminator(nn.Module):
    def __init__(
        self, input_channels: int = 1, out_channels: int = 1, hidden_units: int = 512
    ) -> None:
        super().__init__()
        self.disc = nn.Sequential(
            DiscBlock(1, hidden_units, kernel_size=10, stride=2, input_layer=True),
            DiscBlock(hidden_units, hidden_units * 2),
            DiscBlock(hidden_units * 2, out_channels, 4, 2, False, True),
        )
...
def test_discriminator() -> None:
    disc = Discriminator(input_channels=1, out_channels=1, hidden_units=512)
    fake_image = torch.randn(config.BATCH_SIZE, 1, 28, 28)
    pred = disc(fake_image)

    # Test shape of discriminator output
    assert pred.shape == torch.Size([config.BATCH_SIZE, 1])
